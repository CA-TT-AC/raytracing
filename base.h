...
public:
    ...
    std::shared_ptr<Texture> diffuseTexture; // Texture attribute;
...
public:
    ...
    virtual void getTextureCoordinates(const Vector3 &point, int &u, int &v) const {}
};
...
public:
    ...
    void getTextureCoordinates(const Vector3 &point, int &u, int &v) const;
};
...
public:
    ...
    bool hasTexture() const override { return texture != nullptr; }
    // Vector3 Triangle::computeBarycentricCoordinates(const Vector3& P) const {
    //     Vector3 v0 = v1 - v0, v1 = v2 - v0, v2 = P - v0;
    ...
    // void Triangle::getTextureCoordinates(const Vector3 &point, int &u, int &v) const
    // {
    //     // Assume UV coordinates are defined for each vertex: uv0, uv1, uv2

    //     // // Calculate barycentric coordinates for the point
    //     // Vector3 barycentric = computeBarycentricCoordinates(point);

    //     // // Interpolate UV coordinates based on barycentric coordinates
    //     // float interpolatedU = uv0.u * barycentric.x + uv1.u * barycentric.y + uv2.u * barycentric.z;
    //     // float interpolatedV = uv0.v * barycentric.x + uv1.v * barycentric.y + uv2.v * barycentric.z;

    //     // u = static_cast<int>(interpolatedU * texture->width);
    //     // v = static_cast<int>(interpolatedV * texture->height);
    // }
};
