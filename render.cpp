    ...
    if (json.contains("scene"))
    {
        ...
        for (const auto &shapeJson : sceneJson["shapes"])
        {
            ...
            if (shapeJson.contains("material"))
            {
                ...
                materialPtr->ks = matJson["ks"];
                materialPtr->kd = matJson["kd"];
                materialPtr->specularExponent = matJson["specularexponent"];
                materialPtr->diffuseColor = {matJson["diffusecolor"][0], matJson["diffusecolor"][1], matJson["diffusecolor"][2]};
                materialPtr->ambientColor = materialPtr->diffuseColor * 0.3f;
                materialPtr->specularColor = {matJson["specularcolor"][0], matJson["specularcolor"][1], matJson["specularcolor"][2]};
                materialPtr->isReflective = matJson["isreflective"];
                materialPtr->reflectivity = matJson["reflectivity"];
                materialPtr->isRefractive = matJson["isrefractive"];
                materialPtr->refractiveIndex = matJson["refractiveindex"];

                if (matJson.contains("texture"))
                {
                    std::string texturePath = matJson["texture"];
                    materialPtr->loadDiffuseTexture(texturePath);
                }
            }
            if (type == "sphere")
            {
...
bool Renderer::intersectBinary(const Ray &ray, Shape *shape)
{
    ...
    else if (shape->getType() == "cylinder")
    {
        ...
        if (discriminant >= 0)
        {
            ...
            if ((point1Projection >= heightStart && point1Projection <= heightEnd) || (point2Projection >= heightStart && point2Projection <= heightEnd))
            {
                return true;
            }
        }
...
bool Renderer::intersect(const Ray &ray, Shape *shape, float &distance)
{
    ...
    else if (shape->getType() == "triangle")
    {
        ...
        if (minDistance > 1e-8)
        { // Check for a positive distance to avoid intersections behind the ray origin
            distance = minDistance;
            return true; // Ray intersects the triangle
        }
    }
...
Color getTextureColor(const Vector3 &intersectionPoint, Shape *shape)
{
    if (!shape->material.diffuseTexture)
    {
        return Color(1.0f, 1.0f, 1.0f); // Return white if no texture is present
    }

    // Compute texture coordinates for the intersection point
    int u, v;
    shape->getTextureCoordinates(intersectionPoint, u, v);

    // Fetch the color from the texture at coordinates (u, v)
    return shape->material.diffuseTexture->getColorAt(u, v);
}
Color blendTextureColor(const Color &materialColor, const Color &textureColor)
{
    // Simple blend: average of material color and texture color
    return (materialColor + textureColor) * 0.5f;
    // More complex blends can be implemented based on the material properties
}
